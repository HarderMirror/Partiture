...
model.fit(x_train, y_train, epochs=10, shuffle=True, use_multiprocessing=True, verbose=True)
...
with open("modelNumImages2.json", "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
model.save_weights("modelNumImages2.h5")
...
# x_test = cv2.imread("images/76.jpg",0)
# x_test = np.array(x_test).reshape(1, height, width, 1)
# x_test = np.dot(x_test, 1/255)
# pred = model.predict(x_test,verbose=1)
# print(pred)
